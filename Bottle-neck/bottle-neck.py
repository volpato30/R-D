...
def build_NIN_model(input_var, hidden_neurons=1024, bottle_neck=128):
    ...
    l_b1 = DenseLayer(l_in, num_units=bottle_neck, W=lasagne.init.HeNormal(), nonlinearity=None)
    l_hidden1 = DenseLayer(l_b1, num_units=hidden_neurons, W=lasagne.init.HeNormal(gain='relu'))
    l_b2 = DenseLayer(l_hidden1, num_units=bottle_neck, W=lasagne.init.HeNormal(), nonlinearity=None)
    l_hidden2 = DenseLayer(l_b2, num_units=hidden_neurons, W=lasagne.init.HeNormal(gain='relu'))
    l_out = DenseLayer(l_hidden2, num_units=10, nonlinearity=lasagne.nonlinearities.softmax, W=lasagne.init.HeNormal(gain='relu'))
    return l_out

def train_bottleneck_model(num_epochs=500, hidden_neurons=1024, bottleneck=128, learn_rate=0.1):
...
result = []
for i in range(10):
    temp = train_bottleneck_model(hidden_neurons=8192, bottleneck=K)
    result.append(temp)
result = np.asarray(result)
with open('./result/bottle_neck_{}_result.p'.format(K), 'wb') as f:
